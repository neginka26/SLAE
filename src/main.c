...
int main() {
  int dimension = 3;
  // выделение памяти под основную матрицу и правую часть
  float **matrix = create_matrix(dimension, dimension);
  float **right_part = create_matrix(dimension, 1);
  // заполнение матрицы
  fill_matrix(matrix, dimension, dimension);
  fill_matrix(matrix, dimension, 1);
  // вывод матрицы и правой части
  output_matrix(matrix, dimension, dimension);
  output_matrix(right_part, dimension, 1);
...
// to do: 1) совместить преобразование к трегольному виду с правой частью уравнений
// - complete
// 2) сделать решение СЛАУ (обратный ход) - complete
// 3) сделать выборку по главному элементу столбца и строки